...
public class PermissionController {
  ...
  public List<UserPermissionsDTO> userPermissions(@PathVariable String username) {
    ...
    return user.getPermissions().stream()
        .map(permission -> new UserPermissionsDTO(permission.getId(), permission.getName())).toList();
  }
  ...
  public List<PermissionsUsersDTO> permissionsUsers() {
    ...
    return users.stream().map((user) -> {
      List<UserPermissionsDTO> userPermissions = user.getPermissions().stream()
          .map(permission -> new UserPermissionsDTO(permission.getId(), permission.getName())).toList();
  ...
  public ResponseEntity<Object> createPermissions(@RequestBody @Valid PermissionManagementDTO permissionData) {
    ...
    List<Permission> newPermissions = existentPermissions.stream()
        .map(permission -> permissionRepository.findByName(permission)).toList();
  ...
  public ResponseEntity<Object> deletePermissions(@PathVariable String username,
      @RequestParam List<String> permissions) {
    Boolean userExists = userRepository.existsByUsername(username);
    List<String> existentPermissions = permissions.stream().filter((permission) -> {
      return permissionRepository.existsByName(permission);
    }).toList();
    List<Permission> permissionsToDelete = existentPermissions.stream()
        .map(permission -> permissionRepository.findByName(permission)).toList();
    if (userExists && !existentPermissions.isEmpty()) {
      User user = userRepository.findByUsername(username);
